  ...
  while (true)
  {
    while (joystick.getEvent(&event))
    {
      ...
      if (event.type == JS_EVENT_AXIS && event.number < 20)
      {
        ...
        std::cout << "Axis [" << (int)event.number << "] " << event.getValue() << std::endl;
      }
    }

    if (true)
    {  // fabs(targetSpeed.getLength() - speed.getLength()) < maxAcceleration*0.005) {
      speed = targetSpeed;
    }
    else
    {
    ...
    if (master.robots[ROBOT].isOk())
    {
      ...
      // std::cout << "X: " << speed.x << ", Y: " << speed.y << ", T: " << thetaSpeed << ", Volts: " << voltage <<
      // std::endl;
