...
Manual::Manual(std::string name) : Manager(name)
{
  ...
  registerStrategy("PVC - B - Obstructor", std::shared_ptr<strategy::Strategy>(new strategy::FromRobotBehavior(
                                               [&](double time, double dt) {
                                                 robot_behavior::PVCObstructor* obs =
                                                     new robot_behavior::PVCObstructor();
                                                 obs->declareRobotToObstruct(2, Opponent);
                                                 return std::shared_ptr<robot_behavior::RobotBehavior>(obs);
                                               },
                                               false  // we don't want to define a goal here !
                                               )));
  registerStrategy("PVC - B - Protect Ball", std::shared_ptr<strategy::Strategy>(new strategy::FromRobotBehavior(
                                                 [&](double time, double dt) {
                                                   robot_behavior::ProtectBall* pro = new robot_behavior::ProtectBall();
                                                   return std::shared_ptr<robot_behavior::RobotBehavior>(pro);
                                                 },
                                                 false  // we don't want to define a goal here !
                                                 )));
