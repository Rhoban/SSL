...
Manual::Manual(Ai::AiData& ai_data)
    ...
    register_strategy(
        ...
        std::shared_ptr<Strategy::Strategy>(new Strategy::From_robot_behavior(
            ai_data,
            [&](double time, double dt) {
                Robot_behavior::Defensor* defensor =
    ...
    register_strategy(
        ...
        std::shared_ptr<Strategy::Strategy>(new Strategy::From_robot_behavior(
            ...
            [&](double time, double dt) {
                ...
                return std::shared_ptr<Robot_behavior::RobotBehavior>(
                    passive_defensor);
            },
            false  // we don't want to define a goal here !
            )));
    ...
    register_strategy(
        "two_way_on_width_trip_ally",
        std::shared_ptr<Strategy::Strategy>(new Strategy::From_robot_behavior(
            ai_data,
            [&](double time, double dt) {
                Robot_behavior::Patrol* two_way_trip =
                    Robot_behavior::Patrol::two_way_trip_on_width(ai_data,
                                                                  true);
                return std::shared_ptr<Robot_behavior::RobotBehavior>(
                    two_way_trip);
            },
    ...
    register_strategy(
        "two_way_on_width_trip_opponent",
        std::shared_ptr<Strategy::Strategy>(new Strategy::From_robot_behavior(
            ai_data,
            [&](double time, double dt) {
                Robot_behavior::Patrol* two_way_trip =
    ...
    register_strategy(
        ...
        std::shared_ptr<Strategy::Strategy>(new Strategy::From_robot_behavior(
            ...
            [&](double time, double dt) {
                Robot_behavior::Patrol* two_way_trip =
                    Robot_behavior::Patrol::two_way_trip_on_border(ai_data,
                                                                   false);
                return std::shared_ptr<Robot_behavior::RobotBehavior>(
                    two_way_trip);
            },
    ...
    register_strategy(
        ...
        std::shared_ptr<Strategy::Strategy>(new Strategy::From_robot_behavior(
            ai_data,
            [&](double time, double dt) {
                Robot_behavior::Test_kicker* pt =
                    new Robot_behavior::Test_kicker(ai_data);
                return std::shared_ptr<Robot_behavior::RobotBehavior>(pt);
            },
            false  // we don't want to define a goal here !
            )));
    register_strategy(
        "test_translation_for_pid",
        std::shared_ptr<Strategy::Strategy>(new Strategy::From_robot_behavior(
            ai_data,
            [&](double time, double dt) {
    ...
    register_strategy(
        "test_NW_translation_for_pid",
        std::shared_ptr<Strategy::Strategy>(new Strategy::From_robot_behavior(
            ai_data,
            [&](double time, double dt) {
                Robot_behavior::Patrol* pt =
                    Robot_behavior::Patrol::test_NW_translation_for_pid(
                        ai_data);
                return std::shared_ptr<Robot_behavior::RobotBehavior>(pt);
            },
            false  // we don't want to define a goal here !
    ...
    register_strategy(
        "test_NE_translation_for_pid",
        std::shared_ptr<Strategy::Strategy>(new Strategy::From_robot_behavior(
            ai_data,
            [&](double time, double dt) {
                Robot_behavior::Patrol* pt =
                    Robot_behavior::Patrol::test_NE_translation_for_pid(
                        ai_data);
                return std::shared_ptr<Robot_behavior::RobotBehavior>(pt);
            },
            false  // we don't want to define a goal here !
    ...
    register_strategy(
        "test_SW_translation_for_pid",
        std::shared_ptr<Strategy::Strategy>(new Strategy::From_robot_behavior(
            ai_data,
            [&](double time, double dt) {
                Robot_behavior::Patrol* pt =
                    Robot_behavior::Patrol::test_SW_translation_for_pid(
                        ai_data);
                return std::shared_ptr<Robot_behavior::RobotBehavior>(pt);
            },
            false  // we don't want to define a goal here !
    ...
    register_strategy(
        "test_SE_translation_for_pid",
        std::shared_ptr<Strategy::Strategy>(new Strategy::From_robot_behavior(
            ai_data,
            [&](double time, double dt) {
                Robot_behavior::Patrol* pt =
                    Robot_behavior::Patrol::test_SE_translation_for_pid(
                        ai_data);
                return std::shared_ptr<Robot_behavior::RobotBehavior>(pt);
            },
            false  // we don't want to define a goal here !
    ...
    register_strategy(
        "test_NW_SE_translation_for_pid",
        std::shared_ptr<Strategy::Strategy>(new Strategy::From_robot_behavior(
            ai_data,
            [&](double time, double dt) {
                Robot_behavior::Patrol* pt =
                    Robot_behavior::Patrol::test_NW_SE_translation_for_pid(
                        ai_data);
                return std::shared_ptr<Robot_behavior::RobotBehavior>(pt);
            },
            false  // we don't want to define a goal here !
    ...
    register_strategy(
        "test_SW_NW_translation_for_pid",
        std::shared_ptr<Strategy::Strategy>(new Strategy::From_robot_behavior(
            ai_data,
            [&](double time, double dt) {
                Robot_behavior::Patrol* pt =
                    Robot_behavior::Patrol::test_SW_NW_translation_for_pid(
                        ai_data);
                return std::shared_ptr<Robot_behavior::RobotBehavior>(pt);
            },
            false  // we don't want to define a goal here !
    ...
    register_strategy(
        ...
        std::shared_ptr<Strategy::Strategy>(new Strategy::From_robot_behavior(
            ai_data,
            [&](double time, double dt) {
                Robot_behavior::Example* example =
                    new Robot_behavior::Example(ai_data);
    ...
    register_strategy(
        ...
        std::shared_ptr<Strategy::Strategy>(new Strategy::From_robot_behavior(
            ai_data,
            [&](double time, double dt) {
                Robot_behavior::Beginner::Go_corner* go_corner =
                    new Robot_behavior::Beginner::Go_corner(ai_data);
                return std::shared_ptr<Robot_behavior::RobotBehavior>(
                    go_corner);
            },
            false  // we don't want to define a goal here !
    ...
    register_strategy(
        "Begginer Annotations - Ball position",
        std::shared_ptr<Strategy::Strategy>(new Strategy::From_robot_behavior(
            ai_data,
            [&](double time, double dt) {
                Robot_behavior::Begginer_annotations_ball_position*
                    ball_position =
                        new Robot_behavior::Begginer_annotations_ball_position(
                            ai_data);
                return std::shared_ptr<Robot_behavior::RobotBehavior>(
                    ball_position);
            },
    ...
    register_strategy(
        "Intermediate Prepare to strike",
        std::shared_ptr<Strategy::Strategy>(new Strategy::From_robot_behavior(
            ai_data,
            [&](double time, double dt) {
    ...
    register_strategy(
        ...
        std::shared_ptr<Strategy::Strategy>(new Strategy::From_robot_behavior(
            ai_data,
            [&](double time, double dt) {
                Robot_behavior::StrikerAi* striker_ai =
    ...
    register_strategy(
        ...
        std::shared_ptr<Strategy::Strategy>(new Strategy::From_robot_behavior(
            ai_data,
            [&](double time, double dt) {
                Robot_behavior::PredictFutur* p =
                    new Robot_behavior::PredictFutur(ai_data);
    ...
    int velocity = 2.0;
    register_strategy(
        "test_N_velocity_consign",
        std::shared_ptr<Strategy::Strategy>(new Strategy::From_robot_behavior(
            ai_data,
            [&](double time, double dt) {
                Robot_behavior::Test_velocity_consign* pt =
                    Robot_behavior::Test_velocity_consign::get_N_movement(
                        ai_data, velocity);
                return std::shared_ptr<Robot_behavior::RobotBehavior>(pt);
            },
            false  // we don't want to define a goal here !
    ...
    register_strategy(
        "test_E_velocity_consign",
        std::shared_ptr<Strategy::Strategy>(new Strategy::From_robot_behavior(
            ai_data,
            [&](double time, double dt) {
                Robot_behavior::Test_velocity_consign* pt =
                    Robot_behavior::Test_velocity_consign::get_E_movement(
                        ai_data, velocity);
                return std::shared_ptr<Robot_behavior::RobotBehavior>(pt);
            },
            false  // we don't want to define a goal here !
    ...
    register_strategy(
        "test_W_velocity_consign",
        std::shared_ptr<Strategy::Strategy>(new Strategy::From_robot_behavior(
            ai_data,
            [&](double time, double dt) {
                Robot_behavior::Test_velocity_consign* pt =
                    Robot_behavior::Test_velocity_consign::get_W_movement(
                        ai_data, velocity);
                return std::shared_ptr<Robot_behavior::RobotBehavior>(pt);
            },
            false  // we don't want to define a goal here !
    ...
    register_strategy(
        "test_S_velocity_consign",
        std::shared_ptr<Strategy::Strategy>(new Strategy::From_robot_behavior(
            ai_data,
            [&](double time, double dt) {
                Robot_behavior::Test_velocity_consign* pt =
                    Robot_behavior::Test_velocity_consign::get_S_movement(
                        ai_data, velocity);
                return std::shared_ptr<Robot_behavior::RobotBehavior>(pt);
            },
            false  // we don't want to define a goal here !
    ...
    register_strategy(
        "test_NW_velocity_consign",
        std::shared_ptr<Strategy::Strategy>(new Strategy::From_robot_behavior(
            ai_data,
            [&](double time, double dt) {
                Robot_behavior::Test_velocity_consign* pt =
                    Robot_behavior::Test_velocity_consign::get_NW_movement(
                        ai_data, velocity);
                return std::shared_ptr<Robot_behavior::RobotBehavior>(pt);
            },
            false  // we don't want to define a goal here !
    ...
    register_strategy(
        "test_NE_velocity_consign",
        std::shared_ptr<Strategy::Strategy>(new Strategy::From_robot_behavior(
            ai_data,
            [&](double time, double dt) {
                Robot_behavior::Test_velocity_consign* pt =
                    Robot_behavior::Test_velocity_consign::get_NE_movement(
                        ai_data, velocity);
                return std::shared_ptr<Robot_behavior::RobotBehavior>(pt);
            },
            false  // we don't want to define a goal here !
    ...
    register_strategy(
        "test_SW_velocity_consign",
        std::shared_ptr<Strategy::Strategy>(new Strategy::From_robot_behavior(
            ai_data,
            [&](double time, double dt) {
                Robot_behavior::Test_velocity_consign* pt =
                    Robot_behavior::Test_velocity_consign::get_SW_movement(
                        ai_data, velocity);
                return std::shared_ptr<Robot_behavior::RobotBehavior>(pt);
            },
            false  // we don't want to define a goal here !
    ...
    register_strategy(
        "test_SE_velocity_consign",
        std::shared_ptr<Strategy::Strategy>(new Strategy::From_robot_behavior(
            ai_data,
            [&](double time, double dt) {
                Robot_behavior::Test_velocity_consign* pt =
                    Robot_behavior::Test_velocity_consign::get_SE_movement(
                        ai_data, velocity);
                return std::shared_ptr<Robot_behavior::RobotBehavior>(pt);
            },
            false  // we don't want to define a goal here !
    ...
    register_strategy(
        ...
        std::shared_ptr<Strategy::Strategy>(new Strategy::From_robot_behavior(
            ai_data,
            [&](double time, double dt) {
                Robot_behavior::TestInfra* p =
                    new Robot_behavior::TestInfra(ai_data);
                return std::shared_ptr<Robot_behavior::RobotBehavior>(p);
            },
            false)));
    register_strategy(
    ...
    register_strategy(
        "test_angular_only_relative_velocity_consign",
        std::shared_ptr<Strategy::Strategy>(new Strategy::From_robot_behavior(
            ai_data,
            [&](double time, double dt) {
                Robot_behavior::Test_relative_velocity_consign* p =
                    Robot_behavior::Test_relative_velocity_consign::
                        get_movement_angular_velocity_only(ai_data, M_PI / 2);
                return std::shared_ptr<Robot_behavior::RobotBehavior>(p);
            },
            false  // we don't want to define a goal here !
            )));
    ...
    register_strategy(
        "test_linear_only_relative_velocity_consign",
        std::shared_ptr<Strategy::Strategy>(new Strategy::From_robot_behavior(
            ai_data,
            [&](double time, double dt) {
                Robot_behavior::Test_relative_velocity_consign* p =
                    Robot_behavior::Test_relative_velocity_consign::
                        get_movement_linear_velocity_only(ai_data,
                                                          Vector2d(0, 1));
                return std::shared_ptr<Robot_behavior::RobotBehavior>(p);
            },
            false  // we don't want to define a goal here !
    ...
    register_strategy(
        "test_linear_and_angular_relative_velocity_consign",
        std::shared_ptr<Strategy::Strategy>(new Strategy::From_robot_behavior(
            ai_data,
            [&](double time, double dt) {
                Robot_behavior::Test_relative_velocity_consign* p =
                    Robot_behavior::Test_relative_velocity_consign::
                        get_movement_angular_and_linear_velocity(
    ...
    register_strategy(Strategy::Halt::name, std::shared_ptr<Strategy::Strategy>(
...
void Manual::update(double time) {
    // if( not(
    // get_strategy_<Strategy::Tare_and_synchronize>().is_tared_and_synchronized()
    // ) ){
    //   assign_strategy( Strategy::Tare_and_synchronize::name, time,
    //   get_valid_player_ids() );
    // }
