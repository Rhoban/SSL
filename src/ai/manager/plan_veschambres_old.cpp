  ...
  , last_referee_changement(0)
{
  ...
  offensive_strats[7] = { Strategy::GoalieStrat::name, Strategy::Mur::name, Strategy::Defensive2::name,
                          Strategy::StrikerV2::name };
  offensive_strats[6] = { Strategy::GoalieStrat::name, Strategy::Mur_2::name, Strategy::Defensive2::name,
                          Strategy::StrikerV2::name };
  offensive_strats[5] = { Strategy::GoalieStrat::name, Strategy::Mur_2::name, Strategy::Defensive::name,
                          Strategy::StrikerV2::name };
...
void PlanVeschambres::analyse_data(double time)
{
  ...
  change_team_and_point_of_view(referee.get_team_color(get_team_name()),
                                referee.blue_have_it_s_goal_on_positive_x_axis());
  change_ally_and_opponent_goalie_id(referee.blue_goalie_id(), referee.yellow_goalie_id());
}
...
void PlanVeschambres::choose_a_strategy(double time)
{
  ...
  else if (referee.get_state() == Referee_Id::STATE_RUNNING)
  {
    ...
    {
      if (!Box({ ball_last_position.getX() - 0.10, ball_last_position.getY() - 0.10 },
