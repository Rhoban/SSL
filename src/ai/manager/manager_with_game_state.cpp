...
void ManagerWithGameState::analyse_data(double time)
{
  ...
  change_team_and_point_of_view(game_state.get_team_color(get_team_name()),
                                game_state.blue_have_it_s_goal_on_positive_x_axis());
  change_ally_and_opponent_goalie_id(game_state.blue_goalie_id(), game_state.yellow_goalie_id());
}
