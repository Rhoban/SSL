...
#define PASSIVE_DEFENSOR "passive_defensor" 
    ...
    register_strategy(
        PASSIVE_DEFENSOR, std::shared_ptr<Strategy::Strategy>(
            new Strategy::From_robot_behavior(
                ai_data,
                [&](double time, double dt){
                    Robot_behavior::Passive_defensor* defensor = new Robot_behavior::Passive_defensor(ai_data);
                    defensor->set_barycenter(.5);
                    defensor->set_robot_to_obstacle( 3, Vision::Team::Opponent );
                    return std::shared_ptr<Robot_behavior::RobotBehavior>(defensor);
                }, true //it is a goal
            )
        )
...
void Sebastien::analyse_data(double time){
...
void Sebastien::choose_a_strategy(double time){
    if( referee.edge_entropy() > last_referee_changement ){
        ...
        if( referee.get_state() == Referee_Id::STATE_INIT ){
            ...
            //future_strats = { GOALIE, DEFENSOR1, STRIKER, PASSIVE_DEFENSOR };
            future_strats = { PASSIVE_DEFENSOR };
            declare_and_assign_next_strategies(future_strats);
...
void Sebastien::update(double time){
