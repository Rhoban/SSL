    ...
    last_referee_changement(0)
{
    ...
    register_strategy(
        Strategy::Base_3_gms::name, std::shared_ptr<Strategy::Strategy>(
            new Strategy::Base_3_gms(ai_data) 
        )
    );
    register_strategy(
...
void Base_3_gms::analyse_data(double time){
...
void Base_3_gms::choose_a_strategy(double time){
    if( referee.edge_entropy() > last_referee_changement ){
        ...
        if( referee.get_state() == Referee_Id::STATE_INIT ){
            ...
            future_strats = { Strategy::Base_3_gms::name };
            declare_and_assign_next_strategies(future_strats);
