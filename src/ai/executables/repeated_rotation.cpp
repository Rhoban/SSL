...
int main(int argc, char** argv)
{
  ...
  public:
    Rotation(uint robot_number) : robot_number_(robot_number)
    {
    }

    bool runTask()
    {
      Data::get()->shared_data.final_control_for_robots[robot_number_].control.angular_velocity = 500;
      return true;
    }
  };

  ExecutionManager::getManager().addTask(new Rotation(assigned_robot.getValue()), 1999);
