...
int main(int argc, char** argv)
{
  ...
  std::vector<rhoban_geometry::Point> path{ Data::get()->field.getQuarterCenter(0),
                                            Data::get()->field.getQuarterCenter(1) };

  class NoRotation : public Task
  {
  private:
    uint robot_number_;

  public:
    NoRotation(uint robot_number) : robot_number_(robot_number)
    {
    }

    bool runTask(void)
    {
      Data::get()->shared_data.final_control_for_robots[robot_number_].control.angular_velocity = 0.0;
      return true;
    }
  };

  ExecutionManager::getManager().addTask(
      new robot_behavior::RobotBehaviorTask(assigned_robot.getValue(), new robot_behavior::tests::TestFollowPath(path)),
      1001);

  ExecutionManager::getManager().addTask(new NoRotation(assigned_robot.getValue()), 1999);
