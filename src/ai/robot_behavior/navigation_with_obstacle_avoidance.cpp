...
void Navigation_with_obstacle_avoidance::determine_the_closest_obstacle()
{
  ...
  std::list<std::pair<int, double> > collisions_with_ctrl = ai_data.get_collisions(robot().id(), ctrl.linear_velocity);
  assert(ai_data.constants.security_acceleration_ratio > ai_data.constants.obstacle_avoidance_ratio);
  double ctrl_velocity_norm = ctrl.linear_velocity.norm();
  double time_to_stop = ctrl_velocity_norm /
                        (ai_data.constants.obstacle_avoidance_ratio * ai_data.constants.translation_acceleration_limit);
  ...
  if (not(ignore_the_ball))
  {
    ...
    std::pair<bool, double> collision =
        collision_time(ai_data.constants.robot_radius,
                       robot().get_movement().linear_position(robot().get_movement().last_time()), ctrl.linear_velocity,
                       ball_radius_avoidance, ball().get_movement().linear_position(ball().get_movement().last_time()),
                       ball().get_movement().linear_velocity(ball().get_movement().last_time()), radius_error);
    if (collision.first)
    {
      double time_before_collision = collision.second;
      if (time_before_collision <= time_to_stop and ctrl_velocity_norm > EPSILON_VELOCITY)
...
void Navigation_with_obstacle_avoidance::compute_the_radius_of_limit_cycle()
{
  ...
  assert(ai_data.constants.radius_security_for_collision < ai_data.constants.radius_security_for_avoidance);
  ...
  if (ball_is_the_obstacle)
  {
    assert(not(ignore_the_ball));  // Normally determine_the_closest_obstacle() set ball_is_the_obstacle to false when
                                   // we ignore the ball
    radius_of_limit_cycle = ai_data.constants.radius_ball + ai_data.constants.robot_radius +
                            ai_data.constants.radius_security_for_avoidance;
  }
  else
  {
    if (robot().get_movement().linear_velocity(ai_data.time).norm() <
...
RhobanSSLAnnotation::Annotations Navigation_with_obstacle_avoidance::get_annotations() const
{
  ...
  if (min_time_collision >= 0)
  {
    ...
    {
      annotations.addCircle(ai_data.all_robots.at(closest_robot).second->get_movement().linear_position(ai_data.time),
