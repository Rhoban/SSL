    ...
    const Ai::Ball &ball)
{
    ...
    rhoban_geometry::Point target_position = rhoban_geometry::center_of_cone_incircle(point_to_obstruct, left_post_position, right_post_position, ai_data.constants.robot_radius);
    ...
    if (target_goal_vector.norm() < limit_defense_area_radius or scalar_product(target_goal_vector, point_to_obstruct_goal_vector) < 0)
    {
