...
Matchings gale_shapley_algorithm(
    const std::vector<MAN> & man_set, const std::vector<WOMAN> woman_set,
    ...
    const bool man_rank_is_increasing=true,
    const bool woman_rank_is_increasing=true
){
    Matchings result;

    std::map<
        unsigned int,  // MAN_ids
    ...
    for( unsigned int man_id=0; man_id < man_set.size(); man_id ++ ){
        table_of_man_preferences.emplace(
            man_id, std::vector<
                std::pair<unsigned int, MAN_RANK>
            >( woman_set.size() )
        ); 
        for( unsigned int women_id=0; women_id<woman_set.size(); women_id++ ){
            table_of_man_preferences.at(man_id)[women_id] = std::pair<unsigned int, MAN_RANK>(
                women_id, man_rank( man_set[man_id], woman_set[women_id] )
            );
        }
        std::sort(
    ...
    while( not_treated_mans.size() > 0 ){
        ...
        while( (!affected) and table_of_man_preferences.at(man_id).size()>0 ){
            ...
            if( it==woman_to_man_matching.end() ){
                woman_to_man_matching.emplace(
                    woman_id, std::pair<
                        unsigned int, WOMAN_RANK
                    >(
                        man_id, man_value
                    )
                );
                ...
                woman_rank_is_increasing? 
                    it->second.second < man_value
                    : 
                    it->second.second > man_value
            ){
                ...
                woman_to_man_matching.at( woman_id ) = std::pair<unsigned int, double>(
                    man_id, man_value
                );
                affected = true;
            }else{
    ...
    for( unsigned int man_id=0; man_id<man_set.size(); man_id++ ){
        if(  
            result.man_to_women_matchings.find(man_id)
            == 
            result.man_to_women_matchings.end()
        ){
    ...
    for( unsigned int woman_id=0; woman_id<woman_set.size(); woman_id++ ){
        if(  
            result.women_to_man_matchings.find(woman_id)
            == 
            result.women_to_man_matchings.end()
        ){
