...
std::function<
    double ( const Man& juge, const Woman & women )
> simple_man_rank = [](
    const Man & juge, const Woman & women
){
    return women.value;
};

std::function<
    int ( const Woman& juge, const Man & man )
> simple_woman_rank = [](
    const Woman& juge, const Man & man
){
    return man.value;
};
...
TEST(test_matching, gale_shapley_algorithm__empty_cases){
    {
        ...
        matching::Matchings matchings = matching::gale_shapley_algorithm(
            man_set, woman_set,
            simple_man_rank, simple_woman_rank
        );
...
TEST(test_matching, gale_shapley_algorithm__use_cases_with_struct){
    {
        ...
        EXPECT_EQ(
            ( map2set(matchings.man_to_women_matchings) ),
            (
                std::set<
...
TEST(test_matching, gale_shapley_algorithm__use_cases_with_double){
    {
        ...
        std::function<
            double ( const double& juge, const double & women )
        > simple_man_rank = [](
            const double & juge, const double & women
        ){
            return std::fabs(juge - women);
        };
        std::function<
            double ( const double& juge, const double & man )
        > simple_woman_rank = [](
            const double& juge, const double & man
        ){
            return std::fabs(juge - man);
        };
