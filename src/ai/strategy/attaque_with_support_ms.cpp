...
namespace RhobanSSL {
  namespace Strategy {
    ...
    AttaqueWithSupportMs::AttaqueWithSupportMs(Ai::AiData & ai_data):
      ...
      // )),
      pass_behavior(std::shared_ptr<Robot_behavior::Striker>(
      ...
      fgbm_constante(0.05)
    {
      ...
      machine.add_edge(
        edge_name::fgbm_score_sup_seuil_1_plus_constante,
        state_name::pass_search, state_name::strike_search,
        [this](const Ai::AiData & data, unsigned int run_number, unsigned int atomic_run_number ){
          return fgbm_score_sup_seuil_1_plus_constante();
        }
      );
      machine.add_edge(
        edge_name::fgbm_score_sup_seuil_2_plus_constante,
        state_name::search_pass, state_name::search_strike,
        [this](const Ai::AiData & data, unsigned int run_number, unsigned int atomic_run_number ){
      ...
      machine.add_edge(
        edge_name::db1_sup_db2_plus_constante,
        state_name::pass_search, state_name::search_strike,
        [this](const Ai::AiData & data, unsigned int run_number, unsigned int atomic_run_number ){
          return is_db1_sup_db2_plus_constante();
        }
      );
      machine.add_edge(
        edge_name::db1_plus_constante_inf_db2,
        state_name::search_pass, state_name::strike_search,
        [this](const Ai::AiData & data, unsigned int run_number, unsigned int atomic_run_number ){
