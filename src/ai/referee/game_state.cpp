...
namespace RhobanSSL {
  ...
  bool command_is_(
    const GameStateData & game_state_data,
    unsigned int run_number, unsigned int atomic_run_number
  ){
    return (
      game_state_data.command_is_new()
    ) and (
      game_state_data.current().command() == E
    );
  }
  ...
  bool command_is_one_of_4_(
    ...
    return (
      ...
      ( game_state_data.current().command() == E2 )
      or
      ( game_state_data.current().command() == E3 )
      or
      ( game_state_data.current().command() == E4 )
    );
  }
  ...
  number_of_blue_goals(0)
  {
    ...
    machine_state.add_edge(
      ...
      state_name::stop, state_name::running,
      command_is_<SSL_Referee::FORCE_START>
    );

    machine_state.add_edge(
      edge_name::running_to_stop,
      state_name::running, state_name::stop,
      command_is_<SSL_Referee::STOP>
    );


    machine_state.add_edge(
      edge_name::stop_to_prepare_kickoff,
      state_name::stop, state_name::prepare_kickoff,
      ...
      [&]( const GameStateData & data, unsigned int run_number, unsigned int atomic_run_number ){
        if(
          (game_state_data.current().command() == SSL_Referee::PREPARE_KICKOFF_YELLOW)
        ){
    ...
    machine_state.add_edge(
      edge_name::prepare_kickoff_to_stop,
      state_name::prepare_kickoff, state_name::stop,
      command_is_<SSL_Referee::STOP>
    );

    machine_state.add_edge(
      edge_name::kickoff_to_stop,
      state_name::kickoff, state_name::stop,
      command_is_<SSL_Referee::STOP>
    );

    machine_state.add_edge(
      edge_name::start,
      state_name::prepare_kickoff, state_name::kickoff,
      command_is_<SSL_Referee::NORMAL_START>
    );

    machine_state.add_edge(
      edge_name::ball_move_after_kickoff,
      state_name::kickoff, state_name::running,
      [this](const GameStateData & data, unsigned int run_number, unsigned int atomic_run_number ){
        return (
          ball_is_moving()
     ...
     machine_state.add_edge(
      edge_name::prepare_kickoff_to_halt,
      state_name::prepare_kickoff, state_name::halt,
      command_is_<SSL_Referee::HALT>
    );

    machine_state.add_edge(
      edge_name::kickoff_to_halt,
      state_name::kickoff, state_name::halt,
      command_is_<SSL_Referee::HALT>
    );


    machine_state.add_edge(
      edge_name::stop_to_free_kick,
      state_name::stop, state_name::free_kick,
      command_is_one_of_4_<SSL_Referee::DIRECT_FREE_YELLOW, SSL_Referee::DIRECT_FREE_BLUE, SSL_Referee::INDIRECT_FREE_YELLOW, SSL_Referee::INDIRECT_FREE_BLUE>,
      [&]( const GameStateData & data, unsigned int run_number, unsigned int atomic_run_number ){
        if(
          (game_state_data.current().command() == SSL_Referee::DIRECT_FREE_YELLOW) or
    ...
    machine_state.add_edge(
      edge_name::free_kick_to_stop,
      state_name::free_kick, state_name::stop,
      command_is_<SSL_Referee::STOP>
    );

    machine_state.add_edge(
      edge_name::ball_move_after_free_kick,
      state_name::free_kick, state_name::running,
      [this](const GameStateData & data, unsigned int run_number, unsigned int atomic_run_number ){
        return (
          ball_is_moving()
          && not(command_is_<SSL_Referee::HALT>(data, run_number, atomic_run_number))
    ...
    machine_state.add_edge(
      edge_name::free_kick_to_halt,
      state_name::free_kick, state_name::halt,
      command_is_<SSL_Referee::HALT>
    );


    machine_state.add_edge(
      edge_name::stop_to_penalty,
      state_name::stop, state_name::penalty,
      command_is_one_of_<SSL_Referee::PREPARE_PENALTY_YELLOW, SSL_Referee::PREPARE_PENALTY_BLUE>,
      [&]( const GameStateData & data, unsigned int run_number, unsigned int atomic_run_number ){
        if(
          (game_state_data.current().command() == SSL_Referee::PREPARE_PENALTY_YELLOW)
        ){
    ...
    machine_state.add_edge(
      edge_name::penalty_to_stop,
      state_name::penalty, state_name::stop,
      command_is_<SSL_Referee::STOP>
    );

    machine_state.add_edge(
      edge_name::ball_move_after_penalty,
      state_name::penalty, state_name::running,
      [this](const GameStateData & data, unsigned int run_number, unsigned int atomic_run_number ){
        return (
          ball_is_moving()
          && not(command_is_<SSL_Referee::HALT>(data, run_number, atomic_run_number))
    ...
    machine_state.add_edge(
      edge_name::stop_to_halt,
      state_name::stop, state_name::halt,
      command_is_<SSL_Referee::HALT>
    );
    ...
    machine_state.add_edge(
      edge_name::running_to_halt,
      state_name::running, state_name::halt,
      command_is_<SSL_Referee::HALT>
    );
    ...
    machine_state.add_edge(
      edge_name::goal,
      state_name::stop, state_name::stop,
      command_is_one_of_<SSL_Referee::GOAL_YELLOW, SSL_Referee::GOAL_BLUE >,
      [&]( const GameStateData & data, unsigned int run_number, unsigned int atomic_run_number ){
        if(
          (game_state_data.current().command() == SSL_Referee::GOAL_YELLOW)
        ){
    ...
    machine_state.add_edge(
      edge_name::penalty_to_halt,
      state_name::penalty, state_name::halt,
      command_is_<SSL_Referee::HALT>
    );
    ...
    machine_state.execute_at_each_edge(
        [&](
             std::string edge_id, GameStateData & state_data, GameStateData & edge_data,
             unsigned int run_number, unsigned int atomic_run_number
        ){
             change_stamp += 1;
        }
  ...
  void GameState::save_last_time_stamps(){
    if( game_state_data.last_time < game_state_data.current().packet_timestamp() ){
      ...
      if(
        game_state_data.last_command_counter <
        game_state_data.current().command_counter()
      ){
